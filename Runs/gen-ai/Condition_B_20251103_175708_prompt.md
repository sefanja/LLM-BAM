...
* **(C6) Capability Impact:** Each `Capability` must modify exactly one `BusinessObject`, with one exception: at the leaf level it may modify multiple objects.
* **(C7) Object Relevance:** Each `BusinessObject` must be modified by exactly one `BusinessCapability`, with one exception: at the leaf level, an object may be transformed by multiple capabilities.
